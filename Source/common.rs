...
impl RunTarget {
	pub fn unwrap_core(&self) -> &Module {
		match self {
			...
			RunTarget::Component(_) => {
				panic!("expected a core wasm module, not a component")
			},
		}
	}
	...
	pub fn unwrap_component(&self) -> &Component {
		match self {
			...
			RunTarget::Core(_) => {
				panic!("expected a component, not a core wasm module")
			},
		}
	}
}
...
fn parse_env_var(s:&str) -> Result<(String, Option<String>)> {
	...
	Ok((parts.next().unwrap().to_string(), parts.next().map(|s| s.to_string())))
}
...
impl RunCommon {
	...
	pub fn ensure_allow_precompiled(&self) -> Result<()> {
		if self.allow_precompiled {
			...
			bail!(
				"running a precompiled module requires the \
				 `--allow-precompiled` flag"
			)
		}
	}
	...
	fn ensure_allow_components(&self) -> Result<()> {
		if self.common.wasm.component_model == Some(false) {
			bail!(
				"cannot execute a component without `--wasm component-model`"
			);
		}
	...
	pub fn load_module_contents(
		...
		#[cfg(feature = "component-model")]
		deserialize_component:impl FnOnce() -> Result<Component>,
	) -> Result<RunTarget> {
		Ok(match engine.detect_precompiled(bytes) {
			...
			None => {
				...
				match code.hint() {
					Some(wasmtime::CodeHint::Component) => {
						...
						{
							bail!(
								"support for components was not enabled at \
								 compile time"
							);
						}
					},
			...
			None => {
				let _ = path;
				bail!(
					"support for compiling modules was disabled at compile \
					 time"
				);
			},
		})
					...
					match std::env::var_os(key) {
						Some(val) => {
							val.into_string().map_err(|_| {
								anyhow!(
									"environment variable `{key}` not valid \
									 utf-8"
								)
							})?
						},
...
impl Profile {
	...
	pub fn parse(s:&str) -> Result<Profile> {
		...
		match &parts[..] {
			["perfmap"] => {
				Ok(Profile::Native(wasmtime::ProfilingStrategy::PerfMap))
			},
			["jitdump"] => {
				Ok(Profile::Native(wasmtime::ProfilingStrategy::JitDump))
			},
			["vtune"] => {
				Ok(Profile::Native(wasmtime::ProfilingStrategy::VTune))
			},
			["guest"] => {
				Ok(Profile::Guest {
