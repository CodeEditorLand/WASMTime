...
fn engines() -> Vec<(Engine, IsAsync)> {
	...
	vec![
		...
		(
			Engine::new(config.clone().allocation_strategy(
				InstanceAllocationStrategy::Pooling(pool.clone()),
			))
			.unwrap(),
			IsAsync::NoPooling,
		),
		(Engine::new(config.async_support(true)).unwrap(), IsAsync::Yes),
		(
			Engine::new(config.allocation_strategy(
...
fn host_to_wasm(c:&mut Criterion) {
	for (engine, is_async) in engines() {
		...
		let bench_calls = |group:&mut BenchmarkGroup<'_, WallTime>,
		                   store:&mut Store<()>| {
			// Bench the overhead of a function that has no parameters or
			// results
			bench_host_to_wasm::<(), ()>(
				group,
				store,
			...
			// Bench the overhead of a function that has some parameters and
			// just one result (will use the raw system-v convention on
...
fn bench_host_to_wasm<Params, Results>(
	...
	c.bench_function(
		...
		|b| {
			...
			let mut space =
				vec![ValRaw::i32(0); params.len().max(results.len())];
			b.iter(|| unsafe {
...
fn wasm_to_host(c:&mut Criterion) {
	...
	fn bench_calls(
		...
		untyped
			.func_new("", "nop", FuncType::new(&engine, [], []), |_, _, _| {
				Ok(())
			})
			.unwrap();
		let ty = FuncType::new(
		...
		unsafe {
			...
			unchecked
				...
				.unwrap();
			let instance = if is_async.use_async() {
				run_await(unchecked.instantiate_async(&mut *store, &module))
					.unwrap()
			} else {
	...
	fn bench_instance(
		...
		group.bench_function(
			&format!("core - wasm-to-host - {desc} - nop"),
			|b| {
				let run = instance
...
mod component {
	...
	fn host_to_wasm(c:&mut Criterion) {
		for (engine, is_async) in engines() {
			...
			let instance = if is_async.use_async() {
				run_await(linker.instantiate_async(&mut store, &component))
					.unwrap()
			} else {
	...
	fn bench_host_to_wasm<Params, Results>(
		...
		Params: component::ComponentNamedList
			+ ToComponentVals
			+ component::Lower
			+ Copy
			+ Send
		...
		c.bench_function(
			...
			|b| {
				...
				b.iter(|| {
					let results = if is_async.use_async() {
						run_await(typed.call_async(&mut *store, typed_params))
							.unwrap()
					} else {
						typed.call(&mut *store, typed_params).unwrap()
					};
		...
		c.bench_function(
			...
			|b| {
				...
				let mut results =
					vec![component::Val::U32(0); expected_results.len()];
				b.iter(|| {
					if is_async.use_async() {
						run_await(untyped.call_async(
							&mut *store,
					...
					for (expected, actual) in
						expected_results.iter().zip(&results)
					{
						assert_eq!(expected, actual);
					}
					if is_async.use_async() {
						run_await(untyped.post_return_async(&mut *store))
							.unwrap()
					} else {
	...
	fn wasm_to_host(c:&mut Criterion) {
		...
		for (engine, is_async) in engines() {
			...
			bench_calls(
				&mut c
					.benchmark_group(&format!("{}/hook-sync", is_async.desc())),
				&mut store,
				&component,
				is_async,
			);
		...
		// Given a `Store` will create various instances hooked up to different
		// ways of defining host imports to benchmark their overhead.
		fn bench_calls(
			group:&mut BenchmarkGroup<'_, WallTime>,
			store:&mut Store<()>,
			...
			typed
				...
				.unwrap();
			let instance = if is_async.use_async() {
				run_await(typed.instantiate_async(&mut *store, &component))
					.unwrap()
			} else {
			...
			untyped
				...
				.unwrap();
			let instance = if is_async.use_async() {
				run_await(untyped.instantiate_async(&mut *store, &component))
					.unwrap()
			} else {
		...
		fn bench_instance(
			...
			group.bench_function(
				&format!("component - wasm-to-host - {desc} - nop"),
				|b| {
					let run = instance
						.get_typed_func::<(u64,), ()>(&mut *store, "run-nop")
						.unwrap();
					b.iter_custom(|iters| {
						let start = Instant::now();
						if is_async.use_async() {
							run_await(run.call_async(&mut *store, (iters,)))
								.unwrap();
							run_await(run.post_return_async(&mut *store))
								.unwrap();
						} else {
							run.call(&mut *store, (iters,)).unwrap();
