	...
	c.bench_function("lazy initialization at call", move |b| {
		...
		b.iter_custom(move |iters| {
			(0..iters)
				.into_iter()
				.map(|_| {
					lazy_thread_instantiate(engine.clone(), module.clone())
				})
				.sum()
		})
	});
	...
	c.bench_function("eager initialization", move |b| {
		...
		b.iter_custom(move |iters| {
			(0..iters)
				.into_iter()
				.map(|_| {
					let (init, _call) = eager_thread_instantiate(
						engine.clone(),
						module.clone(),
					);
					init
	...
	c.bench_function("call after eager initialization", move |b| {
		...
		b.iter_custom(move |iters| {
			(0..iters)
				.into_iter()
				.map(|_| {
					let (_init, call) = eager_thread_instantiate(
						engine.clone(),
						module.clone(),
					);
					call
...
/// Creating a store and measuring the time to perform a call is the same
/// behavior in both setups.
...
fn test_setup() -> (Engine, Module) {
	...
	// The module has a memory (shouldn't matter) and a single function which is
	// a no-op.
	let module =
		Module::new(&engine, r#"(module (memory 1) (func (export "f")))"#)
			.unwrap();
	(engine, module)
}
