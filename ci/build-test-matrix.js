...
function supports32Bit(pkg) {
	if (pkg.indexOf("pulley") !== -1) return true;
		...
		for (const bucket of buckets) {
			sharded.push(
				Object.assign({}, config, {
					...
					bucket: members
						.map((c) =>
							bucket.has(c) ? `--package ${c}` : `--exclude ${c}`,
						)
						.join(" "),
				}),
			);
...
async function main() {
	...
	const filtered = FULL_MATRIX.filter((config) => {
		...
		if (
			config.isa &&
			names.includes(`cranelift/codegen/src/isa/${config.isa}`)
		) {
			return true;
		}
		...
		if (names.includes(`cranelift/filetests/filetests/runtests`)) {
			if (config.isa !== undefined) return true;
		}
		...
		// too.
		if (config["32-bit"] === true) {
			if (names.includes("pulley")) return true;
			if (names.includes("fiber")) return true;
		}
