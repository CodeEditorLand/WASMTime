...
fn main() -> Result<()> {
	...
	println!(
		"Define a Wasm module that will mutate local state and then trap."
	);
	...
	let run =
		instance.get_func(&mut store, "run").expect("should have 'run' export");
	let args = &[];
	let results = &mut [];
	...
	let dump = err.downcast_ref::<WasmCoreDump>().expect(
		"should have an attached core dump, since we configured core dumps on",
	);

	println!("Number of memories in the core dump: {}", dump.memories().len());
	...
	println!("Number of globals in the core dump: {}", dump.globals().len());
