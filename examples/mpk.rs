...
struct Args {
	...
	/// slots; see [`Config::static_memory_guard_size`] for more details and
	/// the default value if unset.
	#[arg(long, value_parser = parse_byte_size)]
...
fn probe_engine_size(args:&Args, mpk:MpkEnabled) -> Result<Pool> {
	...
	Ok(Pool { num_memories:search.next(), mapped_bytes })
}
...
impl Pool {
	...
	fn to_string(&self) -> String {
		let human_size =
			ByteSize::b(self.mapped_bytes as u64).to_string_as(true);
		format!("{} memory slots\t{} reserved", self.num_memories, human_size)
	}
...
impl ExponentialSearch {
	fn new() -> Self { Self { growing:true, last:0, next:1 } }
	...
	fn done(&self) -> bool { self.last == self.next }
}
...
fn build_engine(
	args:&Args,
	num_memories:u32,
	enable_mpk:MpkEnabled,
) -> Result<usize> {
	// Configure the memory pool.
	let mut pool = PoolingAllocationConfig::default();
	pool.max_memory_size(args.memory_size);
	pool.total_memories(num_memories).memory_protection_keys(enable_mpk);
