...
mod component {
	use std::{env, fmt::Write, fs, iter, path::PathBuf, process::Command};
	...
	fn write_static_api_tests(out:&mut String) -> Result<()> {
		let seed = if let Ok(seed) = env::var("WASMTIME_FUZZ_SEED") {
			seed.parse::<u64>().with_context(|| {
				anyhow!("expected u64 in WASMTIME_FUZZ_SEED")
			})?
		} else {
		...
		eprintln!(
			"using seed {seed} (set WASMTIME_FUZZ_SEED={seed} in your \
		...
		for _ in 0..TYPE_COUNT {
			...
			let name = component_fuzz_util::rust_type(
				&ty,
				name_counter,
				&mut declarations,
			);
