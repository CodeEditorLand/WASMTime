...
fn main() {
	...
	let ws = Workspace { version:root.version.clone() };
	crates.push(root);
	...
	match &env::args().nth(1).expect("must have one argument")[..] {
		...
		"publish" => {
			...
			println!(
				"    $ git push git@github.com:bytecodealliance/wasmtime.git \
				 vX.Y.Z"
			);
		},
...
fn read_crate(ws:Option<&Workspace>, manifest:&Path) -> Crate {
	...
	Crate { manifest:manifest.to_path_buf(), name, version, publish }
}
...
fn bump_version(krate:&Crate, crates:&[Crate], patch:bool) {
	...
	for line in contents.lines() {
		...
		for other in crates {
			...
			rewritten = true;
			new_manifest
				.push_str(&line.replace(&other.version, &next_version(other)));
			break;
		}
		if !rewritten {
...
fn update_capi_version() {
	...
	for line in contents.lines() {
		if line.starts_with("#define WASMTIME_VERSION \"") {
			new_header
				.push_str(&format!("#define WASMTIME_VERSION \"{version}\""));
		} else if line.starts_with("#define WASMTIME_VERSION_MAJOR") {
			new_header
				.push_str(&format!("#define WASMTIME_VERSION_MAJOR {major}"));
		} else if line.starts_with("#define WASMTIME_VERSION_MINOR") {
			new_header
				.push_str(&format!("#define WASMTIME_VERSION_MINOR {minor}"));
		} else if line.starts_with("#define WASMTIME_VERSION_PATCH") {
			new_header
				.push_str(&format!("#define WASMTIME_VERSION_PATCH {patch}"));
		} else {
...
fn verify(crates:&[Crate]) {
	...
	if Path::new(".cargo").exists() {
		panic!(
			"`.cargo` already exists on the file system, remove it and then \
			 run the script again"
		);
	}
	if Path::new("vendor").exists() {
		panic!(
			"`vendor` already exists on the file system, remove it and then \
			 run the script again"
		);
	}

	let vendor = cmd_output(
		Command::new("cargo").arg("vendor").stderr(Stdio::inherit()),
	);
	...
	fn verify_capi() {
		...
		for line in contents.lines() {
			if line
				.starts_with(&format!("#define WASMTIME_VERSION \"{version}\""))
			{
				count += 1;
			} else if line
				.starts_with(&format!("#define WASMTIME_VERSION_MAJOR {major}"))
			{
				count += 1;
			} else if line
				.starts_with(&format!("#define WASMTIME_VERSION_MINOR {minor}"))
			{
				count += 1;
			} else if line
				.starts_with(&format!("#define WASMTIME_VERSION_PATCH {patch}"))
			{
				count += 1;
			}
		}
